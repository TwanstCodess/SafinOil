    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make('کڕیارانی قەرزدار', $customersWithDebt . ' لە ' . $totalCustomers)
                ->description('ڕێژەی قەرزداران')
                ->descriptionIcon('heroicon-m-users')
                ->color($customersWithDebt > 0 ? 'warning' : 'success'),

            Stat::make('کۆی قەرزەکان', $this->formatMoney($totalCredit))
                ->description('کۆی گشتی قەرز')
                ->descriptionIcon('heroicon-m-credit-card')
                ->color('warning'),

            Stat::make('قەرزی ماوە', $this->formatMoney($remainingCredit))
                ->description('پارەی بەسەر قەرزداران')
                ->descriptionIcon('heroicon-m-clock')
                ->color($remainingCredit > 0 ? 'danger' : 'success'),

            Stat::make('قەرزی بەسەرچوو', $overdueCredits)
